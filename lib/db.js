...
// 更新搜索索引（增量，仅索引新消息）
// convIds[convId] 累计该 term 在此对话所有消息中的出现次数（同消息内 token 已去重，计 1）
function updateSearchIndex(indexStore, convId, messages) {
  // 1. 聚合本批消息中每个 term 的出现次数
  //    必须先聚合再写：否则同一 term 出现在多条消息时，会排队多个 get→put，
  //    后续 get 在前一个 put 完成前已发起、读到旧值，导致计数丢失（应为 N 实为 1）
  ...
  // 2. 每个 unique term 一次 get + 一次 put，避免同 term 多次竞态写
  //    事务保活由 IndexedDB 请求队列自动维持：onsuccess 内同步排队 put，事务不会提前提交
  //    单个 get/put 的 onerror 不单独处理：事务 abort 时由外层 saveConversation 的 tx.onerror 统一 reject
  //    用 forEach 而非 for-of：回调参数天然隔离作用域，不依赖块级 const
  //    （测试加载时 const→var 会破坏 for-of 块级作用域，导致闭包共享变量）
  termCounts.forEach((count, term) => {
    const getReq = indexStore.get(term);
    getReq.onsuccess = () => {
      const entry = getReq.result || { term, convIds: {} };
      entry.convIds[convId] = (entry.convIds[convId] || 0) + count;
    ...
    try {
      ...
      for (const msg of messages) {
        ...
        for (const c of chunks) {
          batchItems.push({
            // embId 格式：${convId}::msg::${msgKey}::chunk::${chunkIdx}
            // convId 本身含 '::'（${platform}::${platformConversationId}），解析端按"从末尾反取固定 4 段"稳定解析
