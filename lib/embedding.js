...
const EmbeddingService = {
  ...
  async init() {
    ...
    console.log(`[Embedding] 初始化完成，模型: ${this._dashscopeModel}，API Key: ${this._dashscopeKey ? '已配置' : '未配置'}`);
  },
  ...
  async setConfig(options = {}) {
    ...
    await saveEmbeddingSettings({ dashscopeKey: this._dashscopeKey, model: this._dashscopeModel });
  },
...
async function getEmbeddingSettings() {
  return new Promise((resolve) => {
    chrome.storage.local.get('embeddingSettings', (result) => {
      resolve(result.embeddingSettings || { dashscopeKey: '', model: 'text-embedding-v4' });
    });
  });
}
