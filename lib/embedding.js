...
const EmbeddingService = {
  ...
  async init() {
    ...
    console.log(`[Embedding] 初始化完成，模型: ${this._dashscopeModel}，API Key: ${this._dashscopeKey ? '已配置' : '未配置'}，内容过滤: think=${this._includeThinking} search=${this._includeSearch}，切片: size=${this._chunkSize} overlap=${this._chunkOverlap}`);
  },
  ...
  // 返回 [{ chunkIdx, vector, total }]，失败的字段 vector 为 null（调用方应跳过）
  ...
  async embedMessageChunks(text) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (vec) {
        results.push({ chunkIdx: i, vector: vec, total: chunks.length });
      }
    }
    return results;
  },
